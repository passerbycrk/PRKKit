...
- (void)setScrollViewContentInsetForInfiniteScrolling {
    UIEdgeInsets currentInsets = self.scrollView.contentInset;
    currentInsets.bottom = self.originalBottomInset + (self.scrollView.showsInfiniteScrolling ? SVInfiniteScrollingViewHeight : 0.0f);
    [self setScrollViewContentInset:currentInsets];
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    // ADD by ZS,防止empty时同时触发pullToRefresh和infiniteToRefresh
    BOOL shouldInfinite = self.scrollView.contentSize.height >= (self.scrollView.bounds.size.height - self.scrollView.contentInset.bottom - self.scrollView.contentInset.top);
    if (!shouldInfinite) {
        self.state = SVInfiniteScrollingStateStopped;
    }else if(self.state != SVInfiniteScrollingStateLoading && self.enabled) {
...
- (void)setState:(SVInfiniteScrollingState)newState {
    ...
    if(hasCustomView) {
        ...
        if (SVInfiniteScrollingStateStopped == newState) {
