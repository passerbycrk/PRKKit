...
static char UIScrollViewPullToRefreshLogoView;

@implementation UIScrollView (SVPullToRefresh)

@dynamic pullToRefreshView, pullToRefreshLogoView, showsPullToRefresh;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    if(!self.pullToRefreshView) {
        ...
        // ADD by ZS
        UIImage *logoImage = [UIImage imageNamed:@"logo-gray.png"];
        UIImageView *logoView = [[UIImageView alloc] initWithImage:logoImage];
        [self addSubview:logoView];
        self.pullToRefreshLogoView = logoView;
...
- (void)setPullToRefreshLogoView:(UIImageView *)pullToRefreshLogoView
{
    [self willChangeValueForKey:@"SVPullToRefreshLogoView"];
    objc_setAssociatedObject(self, &UIScrollViewPullToRefreshLogoView,
                             pullToRefreshLogoView,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    [self didChangeValueForKey:@"SVPullToRefreshLogoView"];
}

- (UIImageView *)pullToRefreshLogoView
{
    return objc_getAssociatedObject(self, &UIScrollViewPullToRefreshLogoView);
}
...
- (void)setShowsPullToRefresh:(BOOL)showsPullToRefresh {
    ...
    // ADD by ZS
    self.pullToRefreshLogoView.centerX = self.width/2.0f;
    self.pullToRefreshLogoView.bottom = showsPullToRefresh?self.pullToRefreshView.top:( - 20.0f);
}
...
- (void)updateDateInfo
{
    ...
    [self setSubtitle:[NSString stringWithFormat:@"最后更新: %@", [formatter stringFromDate:[NSDate date]]]
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGFloat leftViewWidth = 0.0f;//MAX(self.arrow.bounds.size.width,self.activityIndicatorView.bounds.size.width);
        ...
        CGSize titleSize = [PRKTableViewUtil contentString:self.titleLabel.text
        ...
        CGSize subtitleSize = [PRKTableViewUtil contentString:self.subtitleLabel.text
                                           ...
                                           constrainedToSize:CGSizeMake(labelMaxWidth,self.subtitleLabel.font.lineHeight)
                                               lineBreakMode:self.subtitleLabel.lineBreakMode
                                                   alignment:self.subtitleLabel.textAlignment];
        ...
        if(subtitleSize.height > 0){
            ...
            CGFloat titleY = minY;
            // ADD by ZS : 居中显示
            CGFloat maxWidth = (subtitleSize.width > titleSize.width ? subtitleSize.width : titleSize.width);
            self.titleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY, maxWidth, titleSize.height));
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(labelX, titleY + titleSize.height + marginY, maxWidth, subtitleSize.height));
        }else{
        ...
        self.arrow.frame = CGRectMake(arrowX-self.arrow.bounds.size.width,
...
- (SVPullToRefreshArrow *)arrow {
    if(!_arrow) {
		_arrow = [[SVPullToRefreshArrow alloc]initWithFrame:CGRectMake(0, self.bounds.size.height-54+26, 22, 48-26)];
...
- (UILabel *)titleLabel {
    if(!_titleLabel) {
        ...
        _titleLabel.text = NSLocalizedString(@"下拉可以刷新...",);
...
- (void)setLastUpdatedDate:(NSDate *)newLastUpdatedDate {
    ...
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新: %@",), newLastUpdatedDate?[self.dateFormatter stringFromDate:newLastUpdatedDate]:NSLocalizedString(@"",)];
}

- (void)setDateFormatter:(NSDateFormatter *)newDateFormatter {
	dateFormatter = newDateFormatter;
    self.dateLabel.text = [NSString stringWithFormat:NSLocalizedString(@"最后更新: %@",), self.lastUpdatedDate?[newDateFormatter stringFromDate:self.lastUpdatedDate]:NSLocalizedString(@"",)];
}
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ic_pull_refresh_arrow"]];
        imageView.center = CGPointMake(self.width/2.0f, self.height/2.0f);
