...
open class TableView: UITableView {
    ...
    weak open var longPressReorderDelegate: TableViewDelegate!
    ...
    fileprivate func initialize() {
        longPressGestureRecognizer = UILongPressGestureRecognizer(target: self, action: #selector(TableView._longPress(_:)))
...
extension TableView {
    fileprivate func canMoveRowBeginAt(indexPath: IndexPath) -> Bool {
        if dataSource is TableViewDataSource {
            let baseDataSource: TableViewDataSource! = dataSource as! TableViewDataSource
            return (baseDataSource?.responds(to: #selector(TableViewDataSourceProtocol.tableView(_:canBeginMoveRowAtIndexPath:))) == false) || (baseDataSource?.tableView(self, canBeginMoveRowAtIndexPath: indexPath) == true)
        }
        return true
    }
        ...
        if gesture.state == .began {
            if let indexPath = indexPath {
                if var cell = cellForRow(at: indexPath) {
                    ...
                    scrollDisplayLink = CADisplayLink(target: self, selector: #selector(TableView._scrollTableWithCell(_:)))
